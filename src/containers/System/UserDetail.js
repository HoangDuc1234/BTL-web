...
class UserDetail extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      prevProps.userDetail.error !== this.props.userDetail.error &&
    ...
    if (prevProps.userDetail.userInfo !== this.props.userDetail.userInfo) {
      ...
      if (
        ...
        userInfo.UserInfo.img.data &&
        userInfo.UserInfo.img.data.length > 0
      ) {
        try {
          // Chuyển đổi buffer thành binary
          const imageUrl = new Buffer(userInfo.UserInfo.img, "base64").toString(
            "binary"
          );
  ...
  handleSaveChanges = async () => {
    ...
    try {
      ...
      const address_infor = document.querySelector(
        'input[name="address_infor"]'
      ).value;
          ...
          <div className="user-detail-content">
            <div className="user-avatar-section">
              <div
                ...
                style={{
                  backgroundImage: `url(${
                    previewImgURL || "https://avatar.iran.liara.run/public"
                  })`,
                }}
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    updateUserDetail: (userId, userData) =>
      dispatch(updateUserDetail(userId, userData)),
  };
};
